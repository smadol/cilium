...
``--bpf-nat-global-max`` and ``--bpf-policy-map-max``. Using the
``--bpf-map-dynamic-size-ratio`` these upper capacity limits determined based on
the given ratio of the total system memory.
